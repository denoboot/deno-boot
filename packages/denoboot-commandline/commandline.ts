...
// commandline.ts
import ap from "@denoboot/argparse/mod.ts";
import { EventEmitter } from "@denoboot/events/mod.ts";
import Command, { type CommandConfig, type CommandExample, GlobalCommand, type HelpCallback } from "./command.ts";
import type { OptionConfig } from "./option.ts";
import { camelcaseOptionName, getArgParseOptions, getFileName, setByType, setDotProp } from "./utils.ts";
...
class CommandLine extends EventEmitter {
  ...
  parse(
    ...
    if (
      this.options.version && this.showVersionOnExit &&
