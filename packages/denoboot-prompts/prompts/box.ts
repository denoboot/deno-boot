...
const squareSymbols: BoxSymbols = [
  S_BAR_START,
  S_BAR_START_RIGHT,
  S_BAR_END,
  S_BAR_END_RIGHT,
...
export const box = (message = "", title = "", opts?: BoxOptions) => {
  ...
  const width = opts?.width === undefined || opts.width === "auto"
    ? 1
    : Math.min(1, opts.width);
  const hasGuide = (opts?.withGuide ?? settings.withGuide) !== false;
  const linePrefix = !hasGuide ? "" : `${S_BAR} `;
  const formatBorder = opts?.formatBorder ?? defaultFormatBorder;
  const symbols = (opts?.rounded ? roundedSymbols : squareSymbols).map(
    formatBorder,
  );
  const hSymbol = formatBorder(S_BAR_H);
  const vSymbol = formatBorder(S_BAR);
  const linePrefixWidth = fsw(linePrefix);
  const titleWidth = fsw(title);
  ...
  if (opts?.width === "auto") {
    ...
    for (const line of lines) {
      const lineWithPadding = fsw(line) + contentPadding * 2;
  ...
  const truncatedTitle = titleWidth > maxTitleLength
  ...
  output.writeSync(
    encoder.encode(
      `${linePrefix}${symbols[0]}${
        hSymbol.repeat(titlePaddingLeft)
      }${truncatedTitle}${hSymbol.repeat(titlePaddingRight)}${symbols[1]}\n`,
    ),
  );
  ...
  for (const line of wrappedLines) {
    ...
    output.writeSync(
      encoder.encode(
        `${linePrefix}${vSymbol}${" ".repeat(leftLinePadding)}${line}${
  ...
  output.writeSync(
    encoder.encode(
      `${linePrefix}${symbols[2]}${hSymbol.repeat(innerWidth)}${symbols[3]}\n`,
    ),
  );
};
