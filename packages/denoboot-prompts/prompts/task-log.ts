import { getColumns } from "../core/mod.ts";
...
export const taskLog = (opts: TaskLogOptions) => {
  ...
  const isTTY = !isCIFn() && isTTYFn();

  output.writeSync(encoder.encode(`${secondarySymbol}\n`));
  output.writeSync(
    encoder.encode(`${colors.green(S_STEP_SUBMIT)}  ${opts.title}\n`),
  );
  ...
  const clear = (clearTitle: boolean): void => {
    ...
    for (const buffer of buffers) {
      ...
      if (
        result === undefined && buffer.header !== undefined &&
        buffer.header !== ""
      ) {
        text += `\n${buffer.header}`;
      }
    ...
    if (lines > 0) {
      lines += 1;
      output.writeSync(encoder.encode(erase.lines(lines)));
    }
  };
  const printBuffer = (
    buffer: BufferEntry,
