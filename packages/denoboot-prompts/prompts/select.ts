import { SelectPrompt } from "../core/mod.ts";
...
export const select = <Value>(opts: SelectOptions<Value>) => {
  const opt = (
    ...
    switch (state) {
      case "disabled":
        return `${colors.gray(S_RADIO_INACTIVE)} ${
          computeLabel(label, colors.gray)
        }${
      ...
      case "active":
        return `${colors.green(S_RADIO_ACTIVE)} ${label}${
          option.hint ? ` ${colors.dim(`(${option.hint})`)}` : ""
        }`;
      case "cancelled":
        return `${
          computeLabel(label, (str) => colors.strikethrough(colors.dim(str)))
        }`;
      default:
        return `${colors.dim(S_RADIO_INACTIVE)} ${
