import { getColumns, getRows } from "../core/mod.ts";
...
export const limitOptions = <TOption>(
  ...
  // const output = params.output ?? Deno.stdout;
  ...
  let shouldRenderTopEllipsis = maxItems < options.length &&
    slidingWindowLocation > 0;
  let shouldRenderBottomEllipsis = maxItems < options.length &&
  ...
  const slidingWindowLocationWithEllipsis = slidingWindowLocation +
    (shouldRenderTopEllipsis ? 1 : 0);
  const slidingWindowLocationEndWithEllipsis = slidingWindowLocationEnd -
  ...
  if (lineCount > outputMaxItems) {
    ...
    if (followingRemovals > 0) {
      ...
      lineGroups.splice(
        lineGroups.length - followingRemovals,
        followingRemovals,
      );
    }
  }
