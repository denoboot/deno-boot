import { GroupMultiSelectPrompt } from "../core/mod.ts";
...
export const groupMultiselect = <Value>(
  ...
  const opt = (
    ...
    const next = isItem &&
      (options[options.indexOf(option) + 1] ?? { group: true });
    const isLast = isItem && next && next.group === true;
    const prefix = isItem
      ? (selectableGroups ? `${isLast ? S_BAR_END : S_BAR} ` : "  ")
      : "";
    let spacingPrefix = "";
    if (groupSpacing > 0 && !isItem) {
      const spacingPrefixText = `\n${colors.cyan(S_BAR)}`;
      spacingPrefix = `${
        spacingPrefixText.repeat(groupSpacing - 1)
      }${spacingPrefixText}  `;
    }

    if (state === "active") {
      return `${spacingPrefix}${colors.dim(prefix)}${
        colors.cyan(S_CHECKBOX_ACTIVE)
      } ${label}${option.hint ? ` ${colors.dim(`(${option.hint})`)}` : ""}`;
    }
    if (state === "group-active") {
      return `${spacingPrefix}${prefix}${colors.cyan(S_CHECKBOX_ACTIVE)} ${
        colors.dim(label)
      }`;
    }
    if (state === "group-active-selected") {
      return `${spacingPrefix}${prefix}${colors.green(S_CHECKBOX_SELECTED)} ${
        colors.dim(label)
      }`;
    }
    if (state === "selected") {
      const selectedCheckbox = isItem || selectableGroups
        ? colors.green(S_CHECKBOX_SELECTED)
        : "";
      return `${spacingPrefix}${colors.dim(prefix)}${selectedCheckbox} ${
        colors.dim(label)
    ...
    const unselectedCheckbox = isItem || selectableGroups
      ...
      : "";
    return `${spacingPrefix}${colors.dim(prefix)}${unselectedCheckbox} ${
      colors.dim(label)
    }`;
  };
      ...
      switch (this.state) {
        case "submit": {
          ...
          const optionsText = selectedOptions.length === 0
            ? ""
            : `  ${selectedOptions.join(colors.dim(", "))}`;
          return `${title}${colors.gray(S_BAR)}${optionsText}`;
        }
        case "cancel": {
