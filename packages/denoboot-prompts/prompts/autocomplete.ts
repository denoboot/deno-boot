import { AutocompletePrompt } from "../core/mod.ts";
...
export interface AutocompleteOptions<Value>
  extends AutocompleteSharedOptions<Value> {
  /**
      ...
      switch (this.state) {
        case "submit": {
          ...
          const label = selected.length > 0
            ? `  ${colors.dim(selected.map(getLabel).join(", "))}`
            : "";
          return `${headings.join("\n")}\n${colors.gray(S_BAR)}${label}`;
        }

        case "cancel": {
          const userInputText = userInput
            ? `  ${colors.strikethrough(colors.dim(userInput))}`
            : "";
          return `${headings.join("\n")}\n${
            colors.gray(S_BAR)
          }${userInputText}`;
        ...
        default: {
          const guidePrefix = `${
            (this.state === "error" ? colors.yellow : colors.cyan)(S_BAR)
          }  `;
          const guidePrefixEnd =
            (this.state === "error" ? colors.yellow : colors.cyan)(S_BAR_END);
          // Display cursor position - show plain text in navigation mode
          let searchText = "";
          if (this.isNavigating || showPlaceholder) {
            const searchTextValue = showPlaceholder ? placeholder : userInput;
            searchText = searchTextValue !== ""
              ? ` ${colors.dim(searchTextValue)}`
              : "";
          } else {
          ...
          const noResults = this.filteredOptions.length === 0 && userInput
            ? [`${guidePrefix}${colors.yellow("No matches found")}`]
            : [];

          const validationError = this.state === "error"
            ? [`${guidePrefix}${colors.yellow(this.error)}`]
            : [];

          headings.push(
            `${guidePrefix.trimEnd()}`,
            `${guidePrefix}${colors.dim("Search:")}${searchText}${matches}`,
            ...noResults,
            ...validationError,
          );
          ...
          const footers = [
            `${guidePrefix}${colors.dim(instructions.join(" • "))}`,
            `${guidePrefixEnd}`,
          ];
            ...
            : limitOptions({
              ...
              style: (option, active) => {
                ...
                const hint = option.hint && option.value === this.focusedValue
                  ? colors.dim(` (${option.hint})`)
                  : "";

                return active
                  ? `${colors.green(S_RADIO_ACTIVE)} ${label}${hint}`
                  : `${colors.dim(S_RADIO_INACTIVE)} ${
                    colors.dim(label)
                  }${hint}`;
              },
...
export const autocompleteMultiselect = <Value>(
  ...
  const formatOption = (
    ...
    return `${checkbox} ${colors.dim(label)}`;
  };
  ...
  const prompt = new AutocompletePrompt<Option<Value>>({
    ...
    render() {
      ...
      const searchText = this.isNavigating || showPlaceholder
        ? colors.dim(showPlaceholder ? placeholder : userInput) // Just show plain text when in navigation mode
      ...
      switch (this.state) {
        case "submit": {
          return `${title}${colors.gray(S_BAR)}  ${
            colors.dim(`${this.selectedValues.length} items selected`)
          }`;
        }
        case "cancel": {
          return `${title}${colors.gray(S_BAR)}  ${
        ...
        default: {
          const barColor = this.state === "error" ? colors.yellow : colors.cyan;
          // Instructions
          const instructions = [
          ...
          // No results message
          const noResults = this.filteredOptions.length === 0 && userInput
            ? [`${barColor(S_BAR)}  ${colors.yellow("No matches found")}`]
            : [];

          const errorMessage = this.state === "error"
            ? [`${barColor(S_BAR)}  ${colors.yellow(this.error)}`]
            : [];
          ...
          const footerLines = [
            `${barColor(S_BAR)}  ${colors.dim(instructions.join(" • "))}`,
            `${barColor(S_BAR_END)}`,
          ];
