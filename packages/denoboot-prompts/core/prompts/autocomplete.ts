...
function defaultFilter<T extends OptionLike>(
  input: string,
  option: T,
): boolean {
...
> {
  ...
  get userInputWithCursor() {
    if (!this.userInput) {
      return colors.inverse(colors.hidden("_"));
    }
    ...
    return `${s1}${colors.inverse(s2)}${s3.join("")}`;
  }
  ...
  protected _isActionKey(char: string | undefined, key: any): boolean {
    return (
      char === "\t" ||
      (this.multiple &&
        this.isNavigating &&
  ...
  #onKey(_char: string | undefined, key: any): void {
