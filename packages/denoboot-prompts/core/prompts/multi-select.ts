...
export default class MultiSelectPrompt<T extends OptionLike>
  ...
  private toggleAll() {
    ...
    const allSelected = this.value !== undefined &&
      this.value.length === enabledOptions.length;
  ...
  private toggleInvert() {
    ...
    const notSelected = this._enabledOptions.filter((v) =>
  ...
  constructor(opts: MultiSelectOptions<T>) {
    ...
    this.cursor = this.options[cursor].disabled
      ? findCursor<T>(cursor, 1, this.options)
      : cursor;
    this.on("key", (char) => {
      if (char === "a") {
        this.toggleAll();
