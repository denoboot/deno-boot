const isColorSupported =
  !(!!Deno.env.get("NO_COLOR") || Deno.args.includes("--no-color")) &&
  (!!Deno.env.get("FORCE_COLOR") || Deno.args.includes("--color") ||
    Deno.build.os === "windows" ||
    (Deno.stdin.isTerminal() && Deno.env.get("TERM") !== "dumb") ||
    !!Deno.env.get("CI"));

const formatter =
  (open: string, close: string, replace = open) =>
  (input: string | number | null | undefined) => {
    const str = "" + input, index = str.indexOf(close, open.length);
    return ~index
