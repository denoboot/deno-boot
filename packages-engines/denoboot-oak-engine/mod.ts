...
import { DenoBootEnginePlugin } from "@denoboot/engine/plugin-manager.ts";
import { OakEngineAppMiddleware, OakEngineContainer } from "./kernel.ts";
import { OakEngineRouterMiddleware } from "./router.ts";
import { DenoBootWorkerDefinition } from "@denoboot/engine/worker-manager.ts";
import { DenoBootRouteDefinition } from "@denoboot/engine/router.ts";
...
export function defineOakPlugin(
  $: DenoBootEnginePlugin<
    OakEngineAppMiddleware,
...
>(
  $: DenoBootRouteDefinition<
    OakEngineRouterMiddleware,
    OakEngineRouterMiddleware,
    OakEngineContainer<T>
  >,
) {
  return $;
}

export function defineOakPluginWorker(
  $: DenoBootWorkerDefinition<OakEngineContainer>,
) {
  return $;
}

export function defineOakPluginMiddleware<
  S extends Record<PropertyKey, any> = Record<string, any>,
>($: OakEngineAppMiddleware<S>) {
  return $;
}
